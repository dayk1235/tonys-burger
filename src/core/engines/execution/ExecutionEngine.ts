...
import { PLANNING_EVENTS } from "../planning/PlanningEvents";
...
export class ExecutionEngine implements CognitiveEngine {
  ...
  private subscribeToRuntimeEvents(): void {
    ...
    this.eventBus.subscribe(PLANNING_EVENTS.LIFECYCLE_COMPLETED, async (payload) => {
      try {
        const extracted = this.validator.extractFromPlanEvent(payload as unknown as Record<string, unknown>);
        await this.receiveInput(extracted);
      } catch (err) {
        await this.errorReporter.reportEngineError(PLANNING_EVENTS.LIFECYCLE_COMPLETED, err);
      }
    });
  }
}
