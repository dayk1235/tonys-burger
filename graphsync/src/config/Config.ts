...
import type { GraphSyncConfig } from './types.js';

const DEFAULT_CONFIG: Partial<GraphSyncConfig> = {
  version: 1,
  indexing: {
    default_mode: 'fast',
    debounce_ms: 2000,
...
export function loadConfig(configPath?: string): GraphSyncConfig {
  ...
  if (!raw) {
    throw new Error(
      'No .graphsync.yml found in current directory. ' +
        'Create one or specify a path with --config.',
    );
  }

  const parsed = yaml.load(raw) as Partial<GraphSyncConfig>;
  const config = mergeConfig(parsed);

  if (!config.project) {
    throw new Error(
      '.graphsync.yml must specify a "project" name.',
    );
  }
...
function mergeConfig(parsed: Partial<GraphSyncConfig>): GraphSyncConfig {
  return {
